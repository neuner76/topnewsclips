...
const STATUS_STYLES: Record<string, string> = {
  submitted:     'text-muted-foreground bg-muted border-border',
    ...
    <>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-12 text-foreground">
        ...
        <div className="mb-10 border-b-2 border-[oklch(0.52_0.14_196)] pb-6">
          ...
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight mb-3 text-foreground">Recommend a Source</h1>
        ...
        <div className="space-y-12 text-[15px] leading-relaxed text-muted-foreground">
          ...
          {/* Submission form */}
          <section>
            <h2 className="text-lg font-black tracking-tight uppercase mb-5 text-foreground">Submit a recommendation</h2>
            <SubmitForm />
          </section>

          {/* What we evaluate */}
          <section>
            <h2 className="text-lg font-black tracking-tight uppercase mb-2 text-foreground">What we evaluate</h2>
          ...
          {/* Review rules */}
          <section className="p-5 bg-muted border border-border rounded-lg text-sm text-muted-foreground space-y-2">
            ...
            <ul className="space-y-1.5">
              ...
              <li className="flex gap-2"><span className="shrink-0">–</span><span>Editorial decisions are final, but you can dispute a classification via the <Link href="/contact" className="font-semibold text-foreground hover:underline underline-offset-2">contact form</Link>.</span></li>
          ...
          <section>
            <h2 className="text-lg font-black tracking-tight uppercase mb-2 text-foreground">Source changes</h2>
            ...
            <div className="space-y-4">
              {SOURCE_CHANGES.map((c) => (
                <div key={c.source} className="border border-border bg-muted rounded-lg p-4 text-sm">
                  <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                    <span className="font-semibold text-foreground leading-snug">{c.source}</span>
                    <span className="text-xs text-muted-foreground shrink-0">{c.date}</span>
                  </div>
                  <p className="text-muted-foreground leading-relaxed">{c.change}</p>
                  {c.rationale && (
          ...
          {/* Public review log */}
          <section>
            <h2 className="text-lg font-black tracking-tight uppercase mb-2 text-foreground">Review log</h2>
            <p className="text-sm text-muted-foreground mb-6">
            ...
            {log.length === 0 ? (
              ...
              <div className="space-y-4">
                {log.map((s) => (
                  <div key={s.id} className="border border-border bg-muted rounded-lg p-4 text-sm">
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                      <a
                        href={s.channel_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="font-semibold text-foreground hover:underline underline-offset-2 break-all leading-snug"
                      >
                        {s.channel_url}
                      </a>
                    ...
                    <p className="text-muted-foreground leading-relaxed mb-2">{s.reason}</p>
                    ...
                    {s.decision_rationale && (
                      <div className={`mt-3 pt-3 border-t border-border text-xs ${s.status === 'declined' ? 'text-muted-foreground' : 'text-[oklch(0.78_0.12_145)]'}`}>
