...
function footer(unsubUrl: string) {
  return `
    <div style="padding:24px 32px;border-top:1px solid #e5e7eb;text-align:center;">
      <a href="${FEED_URL}" style="font-size:13px;font-weight:700;color:#0e7490;text-decoration:none;">topnewsclips.com/feed</a>
...
function header() {
  return `
    <div style="background:#ffffff;border-bottom:3px solid #0e7490;padding:20px 32px;">
      <a href="${FEED_URL}" style="text-decoration:none;">
        <div style="font-size:22px;font-weight:900;letter-spacing:-0.03em;color:#111827;">TopNewsClips</div>
        <div style="font-size:11px;color:#6b7280;margin-top:2px;">${TAGLINE}</div>
      </a>
    </div>`
}
...
function email1Html(unsubUrl: string) {
  return wrap(`
    ...
    ${p("You just joined a daily briefing built to show the full picture, not just the most profitable one.")}
    ${p("We pull from independent investigations, public-interest reporting, international outlets, visual evidence, and mainstream coverage so you can see what happened, who produced the story, and how widely it has been covered.")}
    ...
    ${p("All of it in 5 minutes: labeled sources, coverage context, international perspective, and useful next steps when a story calls for them.")}
    ${cta("Read today's stories →", FEED_URL)}
    ${p("Your first daily briefing arrives tomorrow morning.")}
    <p style="margin:0 0 4px;font-size:14px;color:#6b7280;">— Top News Clips<br><em>${TAGLINE}</em></p>
...
You just joined a daily briefing built to show the full picture, not just the most profitable one.

We pull from independent investigations, public-interest reporting, international outlets, visual evidence, and mainstream coverage so you can see what happened, who produced the story, and how widely it has been covered.
...
All of it in 5 minutes: labeled sources, coverage context, international perspective, and useful next steps when a story calls for them.
...
function email2Html(unsubUrl: string) {
  return wrap(`
    ...
    ${p("<strong>Global Blindspot</strong> means the rest of the world considers this story significant, but US media has largely skipped it. It is a coverage signal, not a claim that anyone is hiding the story.")}
    ${p("<strong>Global Lens</strong> shows how international journalists are framing stories US outlets are also covering. Same event, different perspective. This is where you can see what a domestic news cycle tends to flatten.")}
    ...
    ${p("<strong>Stay With This Story</strong> gives you careful next steps on eligible stories: track related updates, read the source, share responsibly, or ask a question without turning the site into an action campaign.")}
    ${p("That's it. Now you know how to read the feed.")}
...
Global Blindspot means the rest of the world considers this story significant, but US media has largely skipped it. It is a coverage signal, not a claim that anyone is hiding the story.

Global Lens shows how international journalists are framing stories US outlets are also covering. Same event, different perspective. This is where you can see what a domestic news cycle tends to flatten.
...
Stay With This Story gives you careful next steps on eligible stories: track related updates, read the source, share responsibly, or ask a question without turning the site into an action campaign.

That's it. Now you know how to read the feed.
...
function email25Html(unsubUrl: string) {
  return wrap(`
    ...
    ${p("Most people get their news from whatever the algorithm surfaces. You're now getting the layer underneath — the one built for readers who want the full picture, not the profitable picture.")}
...
Most people get their news from whatever the algorithm surfaces. You're now getting the layer underneath — the one built for readers who want the full picture, not the profitable picture.
...
function referralUrl(referralCode: string) {
  return `${SITE_URL}/feed?ref=${referralCode}&utm_source=email&utm_medium=referral&utm_campaign=email3`
}
...
function email3Html(unsubUrl: string, referralCode: string) {
  ...
  return wrap(`
    ...
    ${p("They get it free. You help build a daily briefing where every source is labeled, broader blind spots are easier to see, and readers are pointed back to evidence instead of outrage.")}
...
They get it free. You help build a daily briefing where every source is labeled, broader blind spots are easier to see, and readers are pointed back to evidence instead of outrage.
